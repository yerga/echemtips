...
| Target is already running | LabVIEW or a prior host still has the FPGA VI active. eChemTips deliberately refuses attachment. | Put hardware in a safe state, close the other host, reset/reinitialize through the approved NI procedure, then reconnect. Do not bypass the check. |
