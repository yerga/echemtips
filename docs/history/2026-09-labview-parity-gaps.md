...
Approach + CV and Scan Hopping + CV are deliberately staged. **Updated after the September 23 hardware tests:** approaches use type-2 stop-on-feedback to avoid the session-wide one-shot EndCurrentLine latch. Baseline-relative operation first executes a 25 ms stationary hold, averages up to the final 16 current samples in Python, and converts the requested Δi into an absolute threshold. Type 8 is not used. Settling/CV/I–t/retract follows only after acknowledged approach completion and the final sample drain. No-contact endpoint holds are explicitly classified and cannot authorize surface work. See [the current shared-host protocol](../SHARED_HOST_SERVICES.md#contact-pause-protocol). The post-contact settling interval uses one or more FPGA hold waypoints and can be zero.
