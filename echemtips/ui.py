...
class ScanHoppingCVPage(ManagedExperimentPage):
    ...
    def __init__(self, app: "EChemTipsApp") -> None:
        ...
        self.z_plot = Plot("Z vs time", "Z (µm)", (COLORS["accent"],), app.settings.display_max_points, rolling_window_s=120); self.current_plot = Plot("Current vs time", "Current 1 (nA)", (COLORS["blue"],), app.settings.display_max_points, rolling_window_s=120)
        tl.addWidget(_plot_card("Z position", "Rolling 120 s view; the complete scan remains recorded.", self.z_plot), 1); tl.addWidget(_plot_card("Current 1", "Rolling 120 s view; the complete scan remains recorded.", self.current_plot), 1); self.visual_tabs.addTab(traces, "Experiment traces")
