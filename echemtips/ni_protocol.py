...
class SampleDecoder:
    ...
    def decode(self, words: list[int] | tuple[int, ...]) -> Sample:
        ...
        return Sample(
            ...
            voltage1_v=s.polarity_factor * raw_to_voltage1(words[3], s.command_voltage_ratio),
            voltage2_v=s.polarity_factor * raw_to_adc_voltage(words[4]),
            current1_na=s.polarity_factor * raw_to_current(words[5], s.current1_v_per_na),
            current2_na=s.polarity_factor * raw_to_current(words[6], s.current2_v_per_na),
